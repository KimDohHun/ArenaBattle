...
EBTNodeResult::Type UBTTask_TurnToTarget::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)  //ExecuteTask는 태스크가 호출될 때 항상 호출되는 것이다. 
{
	...
	ABCharacter->SetActorRotation(FMath::RInterpTo(ABCharacter->GetActorRotation(), TargetRot, GetWorld()->GetDeltaSeconds(), 2.0f));  //RInterpTo보간(수신된 신호에 있어서 송신기에서의 압축, 전송 도중에 감쇠, 수신 회로에서의 판별 작용 등에 의해 약해진, 혹은 수신 누락된 단위 길이의 신호 요소를 동기적으로 회복할 수 있는 특성을 가진 수신법. 신호 보간이라고도 한다. )한다. RInterpTo는 현재위치에서 타깃쪽으로 방향을 반환. 여기에 인자로 속도를 RInterpTospeed를 받음. 
